...
def train(opt):
    ...
    log = open(f'{opt.save_dir}/{opt.exp_name}/log_dataset.txt', 'a')
    ...
    """ final options """
    # print(opt)
    with open(f'{opt.save_dir}/{opt.exp_name}/opt.txt', 'a') as opt_file:
        opt_log = '------------ Options -------------\n'
        args = vars(opt)
    ...
    while(True):
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            # for log
            with open(f'{opt.save_dir}/{opt.exp_name}/log_train.txt', 'a') as log:
                model.eval()
                with torch.no_grad():
                ...
                if current_accuracy > best_accuracy:
                    ...
                    torch.save(model.state_dict(), f'{opt.save_dir}/{opt.exp_name}/best_accuracy.pth')
                if current_norm_ED > best_norm_ED:
                    best_norm_ED = current_norm_ED
                    torch.save(model.state_dict(), f'{opt.save_dir}/{opt.exp_name}/best_norm_ED.pth')
        ...
        if (iteration + 1) % 1e+5 == 0:
            torch.save(
                model.state_dict(), f'{opt.save_dir}/{opt.exp_name}/iter_{iteration+1}.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./saved_models/', help='save dir')
    ...
    os.makedirs(f'{opt.save_dir}/{opt.exp_name}', exist_ok=True)
